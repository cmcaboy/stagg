...
*/
  ...
  const generateNewQueue = async (id,OppositeGender,lat,lon,radius,exceptionsParam) => {
    ...
    console.log('radius: ',radius);
        ...
        const likeList = await db.collection(`users/${id}/likes`).get();
        ...
        const dislikeList = await db.collection(`users/${id}/dislikes`).get();
  ...
  exports.getQueue = functions.https.onRequest(async (req, res) => {
    ...
    try {
      currentList = await db.collection(`users/${id}/queue`).get();
